...
def test_guitar_note_technical_string_fret_survive_round_trip():
    """Guitar notes with fingering must export string/fret in <technical> after inpaint."""
    import xml.etree.ElementTree as ET
    from src.api.canonical import CanonicalScore, GuitarFingering, Measure, Part, PartInfo, ScoreHeader
    from src.api.render.musicxml import canonical_score_to_musicxml
            ...
            Part(
                ...
                events=[
                    Event(
                        ...
                        voice_id=0,
                        pitch_midi=60,
                        fingering=GuitarFingering(string_index=2, fret=10),
                    )
                ],
            )
        ],
    ...
    assert note_el.findtext("./notations/technical/fret") == "10"
